...
class DirHelper
{
    ...
    public function login()
    {
        ...
        if (is_wp_error($user)) {
            echo json_encode(array('loggedin' => false, 'message' => __('Wrong username or password.', 'findbiz-core')));
        } else {
            wp_set_current_user($user->ID);
            wp_set_auth_cookie($user->ID, $keep_signed_in);
            echo json_encode(array('loggedin' => true, 'message' => __('Login successful', 'findbiz-core')));
        }
        exit();
    }
    ...
    public static function review_price()
    {
        ...
        $b_hours = $bdbh ? atbdp_sanitize_array($bdbh) : [];
